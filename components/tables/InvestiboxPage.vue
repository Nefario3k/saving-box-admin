<template>
  <div>
      ...
      <template v-slot:item.user="{ item }">
        <div class="user_content">
          <div class="user_content_details">
            <span @click="showPanel('viewInvestibox')">
              {{ item.user }}
            </span>
          </div>
        </div>
        ...
        <div class="user_content_details" style="justify-content: center">
          <v-menu offset-y min-width="156">
            ...
            <v-list class="dropdown__list">
              <v-list-item
                @click="showPanel('viewInvestibox')"
    ...
    <ViewInvestibox @showEdit="showPanel('edit')" ref="viewInvestibox" />
    <ViewDisableLoanPage ref="disableLoan" />
    <EditInvestiboxPage @continueStep="showPanel('editContd')" ref="edit" />
    <EditInvestiboxPageContd ref="editContd" />
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      headers: [
        {
          text: "INVESTIBOX TITLE",
          align: "start",
          sortable: false,
          value: "user",
        },
        { text: "CATEGORY", value: "category" },
        { text: "UNITS", value: "email" },
        { text: "INVESTORS", value: "balance" },
        { text: "INTEREST", value: "payment" },
        { text: "STATUS", value: "due" },
        { text: "LAST EDITED", value: "status" },
        { text: "ACTION", value: "action" },
      ],
      admin: [
        {
          id: 1,
          user: "Poultry Farming Wuye Abj.",
          email: "200/250",
          balance: 20,
          payment: "10%",
          due: "Active",
  ...
  methods: {
    showPanel(type) {
      switch (type) {
        case "viewInvestibox":
          this.$refs.viewInvestibox.showPanel();
          break;
        case "edit":
          this.$refs.edit.showPanel();
          break;
        case "editContd":
          this.$refs.editContd.showPanel();
          break;
