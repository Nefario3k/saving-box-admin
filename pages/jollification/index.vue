    ...
    <v-row class="row__content">
      <div class="col-12">
        <v-card color="#fff" class="profile__card not__profile__card">
          <header>
            <span>Jollification <span class="value">(4)</span></span>
            <v-btn
              @click="showPanel('createJolly')"
              class="Btn"
              color="var(--primary-color)"
              ><span>Create Jollification</span></v-btn
            >
          </header>
          ...
          <SearchArea placeholder="Search by Title Name, and Category" />
          <TablesJollyPage />
        </v-card>
      </div>
    </v-row>
    <CreateJolly
      @continueStep="showPanel('createJollyContinue')"
      ref="createJolly"
    />
    <CreateJollyContd
      @showAddJolly="showPanel('addJolly')"
      ref="createJollyContinue"
    />
    <CreateJollyItem
      @continueItemStep="showPanel('createJollyContinue')"
      ref="addJolly"
    />
...
export default {
  data() {
    return {
      cardDetails: [
        {
          title: "Total Jolly Amount",
          value: "₦240,000,000",
          details: "",
          color: "var(--card-eight)",
        },
        {
          title: "Total Active Jollification ",
          value: "24",
          details: "",
          color: "var(--card-nine)",
        },
        {
          title: "Completed Jollification",
          value: "19",
          details: "",
          color: "var(--card-ten)",
        },
        {
          title: "Relinquished Jolly",
          value: "09",
          details: "",
  ...
  methods: {
    showPanel(type) {
      switch (type) {
        case "createJolly":
          this.$refs.createJolly.showPanel();
          break;
        case "createJollyContinue":
          this.$refs.createJollyContinue.showPanel();
          break;
        case "createRole":
          this.$refs.createRole.showPanel();
          break;
        case "addJolly":
          this.$refs.addJolly.showPanel();
          break;
